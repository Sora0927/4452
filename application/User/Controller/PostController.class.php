...
class PostController extends HomeBaseController {
	...
	function add_post(){
		if (IS_POST) {
			...
			$_POST['post']['post_author']=$_SESSION["user"]['id'];//get_current_admin_id();
			$article=I("post.post");
	...
	private  function _lists($status=1){
		...
		$where_ands=empty($term_id)?array("a.status=$status and b.post_author=".$_SESSION["user"]['id']):array("a.term_id = $term_id and a.status=$status and b.post_author=".$_SESSION["user"]['id']);
	//	and post_author=".$_SESSION["user"]['id']
	//array_push($where_ands, "post_author"=>$_SESSION["user"]['id']);
		$fields=array(
